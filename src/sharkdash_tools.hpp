...
namespace Tools {
	...
	const T& safeVal(const std::unordered_map<K, T>& map, const K& key, const T& fallback = T{}) {
		if (auto it = map.find(key); it != map.end()) {
			...
			Logger::error("safeVal() called with invalid key: [{}] (Compile sharkdash with DEBUG=true for more extensive logging!)", key);
			return fallback;
		}
	};
#endif
	...
	const T& safeVal(const std::vector<T>& vec, const size_t& index, const T& fallback = T{}) {
		if (index < vec.size()) {
			...
			Logger::error("safeVal() called with invalid index: [{}] (Compile sharkdash with DEBUG=true for more extensive logging!)", index);
			return fallback;
		}
	};
#endif
