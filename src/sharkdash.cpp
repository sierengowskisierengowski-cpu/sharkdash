...
#include "sharkdash_cli.hpp"
#include "sharkdash_config.hpp"
#include "sharkdash_draw.hpp"
#include "sharkdash_input.hpp"
#include "sharkdash_log.hpp"
#include "sharkdash_menu.hpp"
#include "sharkdash_shared.hpp"
#include "sharkdash_theme.hpp"
#include "sharkdash_tools.hpp"
...
//* --------------------------------------------- Main starts here! ---------------------------------------------------
[[nodiscard]] auto sharkdash_main(const std::span<const std::string_view> args) -> int {
	...
	if (Global::real_uid != Global::set_uid) {
		if (seteuid(Global::real_uid) != 0) {
			...
			Global::exit_error_msg = "Failed to change effective user ID. Unset sharkdash SUID bit to ensure security on this system. Quitting!";
			clean_quit(1);
		}
	}
...

	{
		...
		if (config_dir.has_value()) {
			...
			if (cli.config_file.has_value()) {
				...
				Config::conf_file = Config::conf_dir / "sharkdash.conf";
			}
	...
	//? Try to find global sharkdash theme path relative to binary path
#ifdef __linux__
	{ 	std::error_code ec;
	...
	if (std::error_code ec; not Global::self_path.empty()) {
		Theme::theme_dir = fs::canonical(Global::self_path / "../share/sharkdash/themes", ec);
	...
	if (Theme::theme_dir.empty()) {
		for (auto theme_path : {"/usr/local/share/sharkdash/themes", "/usr/share/sharkdash/themes"}) {
	...
	if (not Term::init()) {
		Global::exit_error_msg = "No tty detected!\nSharkdash needs an interactive shell to run.";
		clean_quit(1);
	}
