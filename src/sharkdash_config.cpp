...
//* Functions and variables for reading and writing the sharkdash config file
namespace Config {
	...
	const vector<array<string, 2>> descriptions = {
		{"color_theme", 		"#* Name of a Sharkdash/bpytop/bashtop formatted \".theme\" file, \"Default\" and \"TTY\" for builtin themes.\n"
								"#* Themes should be placed in \"../share/sharkdash/themes\" relative to binary or \"$HOME/.config/sharkdash/themes\""},
		...
		{"log_level", 			"#* Set loglevel for \"~/.local/state/sharkdash.log\" levels are: \"ERROR\" \"WARNING\" \"INFO\" \"DEBUG\".\n"
		...
		fs::path config_dir;
		{
			...
			if (const auto xdg_config_home = std::getenv("XDG_CONFIG_HOME"); xdg_config_home != nullptr) {
				if (fs::exists(xdg_config_home, error)) {
					config_dir = fs::path(xdg_config_home) / "sharkdash";
				}
			} else if (const auto home = std::getenv("HOME"); home != nullptr) {
				error.clear();
				if (fs::exists(home, error)) {
					config_dir = fs::path(home) / ".config" / "sharkdash";
				}
				if (error) {
	...
	auto get_log_file() -> std::optional<fs::path> {
		return get_xdg_state_dir().transform([](auto&& state_home) -> auto { return state_home / "sharkdash.log"; });
	}

	auto current_config() -> std::string {
		auto buffer = std::string {};
		fmt::format_to(std::back_inserter(buffer), "#? Config file for sharkdash v.{}\n", Global::Version);
